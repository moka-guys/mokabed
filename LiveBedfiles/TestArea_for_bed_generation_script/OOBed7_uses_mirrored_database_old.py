...
class Bedfile:
	...
	def mergeboundaries(self):
		...
		for index, gene in bed[bed.columns[0:1]].itertuples():
			mergecds = []
			mergeexon = []
			namelist = []
		...
		#for index, gene in bed[[0]].itertuples():
			...
			for posexons in genepos:
				...
				namelist.append(posexons.name.encode('ascii', 'ignore'))
	...
	def useaccessions(self):
		...
		#for index, gene in bed[[0]].itertuples():
			...
			self.flankingregion(geneposition = genepos, positionsexons= posexons, positionscds = poscds)
